...
No new npm packages: the backend still uses `free-human-design` and `tz-lookup`.
...
4. The full chart page reads the object, fills in the text and sends the bodygraph only the four fields it draws. The PDF button still receives the whole chart plus `_ownerName` (now the full name), `_detailLine` and `_definitionCount`.
